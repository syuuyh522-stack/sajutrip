    ...
    <main style={{ maxWidth: 460, margin: '0 auto', padding: '24px 22px 92px', minHeight: '100dvh' }}>
      ...
      <header style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 20 }}>
        <div style={{ flex: 1 }}>
          ...
          <h1 style={{ fontSize: 20, fontWeight: 600, margin: 0 }}>{t.my.title}</h1>
          {signedUp && nickname && (
            ...
            {ORDER.map((el) => {
              ...
              return (
                <div key={el} style={{ flex: 1, borderRadius: 14, overflow: 'hidden', border: on ? `1.5px solid ${EL_INK[el]}` : '1px solid var(--glass-brd)', background: 'var(--color-surface)', position: 'relative' }}>
                  ...
                  {got > 0 && (
                    <span style={{ position: 'absolute', top: 4, right: 4, fontSize: 13, fontWeight: 700, color: EL_INK[el], background: 'rgba(255,255,255,.85)', borderRadius: 'var(--radius-pill)', padding: '1px 6px', fontVariantNumeric: 'tabular-nums' }}>
                      +{got}
                    </span>
                  )}
                  <div style={{ fontSize: 13, fontWeight: on ? 700 : 500, textAlign: 'center', padding: '6px 0', color: on ? EL_INK[el] : 'var(--color-text-muted)' }}>{name}</div>
                </div>
              );
            })}
          </div>
        ...
        {state.items.length > 0 && (
          <Link href={{ pathname: '/plan', query }} style={{ textDecoration: 'none', color: 'inherit' }}>
            <div style={{ border: '1px solid var(--line)', borderRadius: 14, padding: 14 }}>
              <div style={{ fontSize: 16, fontWeight: 600 }}>{t.my.savedTrip}</div>
              <div style={{ fontSize: 13, color: 'var(--muted)', marginTop: 2 }}>{state.start || '—'} · {state.items.length} {state.items.length === 1 ? t.my.placeOne : t.my.savedTripDesc}</div>
            </div>
          </Link>
        )}
