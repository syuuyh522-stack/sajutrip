...
const en: Dictionary = {
  ...
  character: {
    wood: { label: 'The Grower', desc: 'Your day stem is Wood — a warm, growing spirit who reaches upward and helps others rise.' },
    fire: { label: 'The Spark', desc: 'Your day stem is Fire — bright, passionate, the one who lights up any room.' },
    earth: { label: 'The Anchor', desc: 'Your day stem is Earth — grounded and steady, the person everyone leans on.' },
    metal: { label: 'The Blade', desc: 'Your day stem is Metal — sharp, principled, clear about what matters.' },
    water: { label: 'The Flow', desc: 'Your day stem is Water — deep and adaptable, moving wisely around every obstacle.' },
  },
  kstar: {
  ...
  legal: {
    ...
    back: 'Back',
    privacyBody: [
      'SajuTrip collects only the minimum information needed to generate your element reading: gender and date of birth. If you create a profile you may also pick a nickname, which stays on your device. We do not ask for your legal name, email, phone number, or precise location.',
      'During this proof-of-concept phase, your inputs and saved itineraries are stored only in your browser (localStorage). We run no user database, so clearing your browser data removes them completely. Two things do leave your browser, and both are described below: the birth-date calculation and anonymous usage events.',
      ...
      'To see how the service is used we log anonymous usage events — page views and funnel steps such as finishing a reading or saving an itinerary. Each event carries a random identifier kept in your browser, along with context such as the gender you selected and the elements you were given. It never carries your nickname, contact details, or birth date. Clearing your browser data resets the identifier.',
    ...
    termsBody: [
      ...
      'Place information (names, images, congestion, festivals) comes from Korea Tourism Organization open APIs and may change or contain errors. Travel videos are surfaced from YouTube and play there under its own terms. Always confirm details with the venue before visiting.',
...
const ko: Dictionary = {
  ...
  character: {
    wood: { label: '자라나는 사람', desc: '당신의 일간은 목(木) — 위로 뻗고 주변을 함께 키우는 따뜻한 성장형.' },
    fire: { label: '빛나는 사람', desc: '당신의 일간은 화(火) — 밝고 열정적이며 어디서든 분위기를 밝히는 사람.' },
    earth: { label: '든든한 사람', desc: '당신의 일간은 토(土) — 안정적이고 믿음직해 모두가 기대는 중심.' },
    metal: { label: '벼려진 사람', desc: '당신의 일간은 금(金) — 예리하고 원칙 있으며 중요한 걸 분명히 아는 사람.' },
    water: { label: '흐르는 사람', desc: '당신의 일간은 수(水) — 깊고 유연해 어떤 장애물도 지혜롭게 돌아가는 사람.' },
  },
  kstar: {
    title: '나의 K-star',
    forFun: '재미로',
  ...
  legal: {
    ...
    back: '뒤로',
    privacyBody: [
      '사주트립은 오행 프로필 산출에 필요한 최소 정보인 성별과 생년월일만 수집합니다. 프로필을 만들면 닉네임을 정할 수 있고, 닉네임은 이용자 기기에만 남습니다. 실명·이메일·전화번호·정밀 위치 정보는 요구하지 않습니다.',
      'POC 기간 동안 입력값과 저장한 일정은 이용자의 브라우저(localStorage)에만 저장됩니다. 별도의 회원 데이터베이스를 두지 않으므로 브라우저 데이터를 삭제하면 완전히 제거됩니다. 브라우저 밖으로 나가는 것은 두 가지뿐이며, 아래에서 각각 설명합니다 — 생년월일 역법 계산과 익명 사용 이벤트입니다.',
      '생년월일은 전통 역법 기둥(간지) 계산을 위해 한국천문연구원(KASI) 공공 API로만 전송되며, 계산 후 사주트립이 별도로 저장하지 않습니다.',
      '이용 흐름을 파악하기 위해 익명 사용 이벤트를 기록합니다 — 페이지 조회와 사주 결과 확인·일정 저장 같은 퍼널 단계입니다. 각 이벤트에는 브라우저에 보관되는 임의의 식별자와, 선택한 성별·산출된 오행 같은 맥락 정보가 함께 담깁니다. 닉네임·연락처·생년월일은 담기지 않습니다. 브라우저 데이터를 삭제하면 식별자도 초기화됩니다.',
    ...
    termsBody: [
      ...
      '장소 정보(명칭·이미지·혼잡도·축제)는 한국관광공사 공공 API에서 제공받으며 변경되거나 오류가 있을 수 있습니다. 여행자 영상은 YouTube에서 불러오며 재생은 YouTube 약관에 따릅니다. 방문 전 반드시 현장 정보를 확인해 주세요.',
