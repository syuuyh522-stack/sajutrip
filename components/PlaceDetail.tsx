...
// 바텀시트 프레젠테이션(PO 피드백 #6): 배경 페이지를 유지한 채 덮는다.
// mode='modal' = 인터셉트 라우트(app/@modal/(.)place/[contentId])에서 사용 — 이전 페이지가 뒤에 그대로 남는다.
// mode='page'  = 직접 진입/새로고침 폴백(app/place/[contentId]/page.tsx) — 라우트 자체는 그대로 유지(딥링크·Phase 2 호환).
...
export function PlaceDetail({ mode }: { mode: 'page' | 'modal' }) {
  ...
  // 시트 바깥 상자 — modal: 뷰포트 대비 높이 상한(내부만 스크롤). page: 남은 세로 공간을 채움(부모가 100dvh 고정).
  const sheetOuterStyle: React.CSSProperties = {
    display: 'flex', flexDirection: 'column', borderRadius: '20px 20px 0 0', overflow: 'hidden',
    background: 'var(--color-surface)', boxShadow: '0 -12px 40px rgba(43,42,51,.18)',
    ...(mode === 'modal' ? { maxHeight: '92dvh', width: '100%' } : { flex: 1, minHeight: 0 }),
  };
  // 스크롤 본문 — 헤더(이미지+닫기)는 flex:0 0 auto로 고정, 본문만 overflowY:auto로 스크롤.
  const scrollBodyStyle: React.CSSProperties = { padding: '18px 22px 40px', overflowY: 'auto', WebkitOverflowScrolling: 'touch', flex: '1 1 auto', minHeight: 0 };
  ...
  const body = (
    <>
      ...
      {!place && !notFound && (
        <div className="pdp-sheet" aria-busy="true" style={sheetOuterStyle}>
          <div className="skeleton" style={{ height: 220, flex: '0 0 auto' }} />
          <div style={{ ...scrollBodyStyle, padding: '18px 22px', display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      {place && (
        <div className="pdp-sheet" style={sheetOuterStyle}>
          <div style={{ height: 220, flex: '0 0 auto', position: 'relative', background: place.image ? `center/cover no-repeat url(${place.image})` : (element ? elGradient(element) : 'var(--color-metal)') }}>
            {/* 닫기 — 시트 좌상단 (§8 탭타깃). 진입 출처로 복귀(back), 직접 진입 시 explore 폴백. 헤더가 flex:0 0 auto라 스크롤해도 항상 고정 노출. */}
            <button
              type="button"
              onClick={goBack}
  ...
  if (mode === 'modal') {
    ...
    return (
      <div
        role="presentation"
        onClick={goBack}
        style={{ position: 'fixed', inset: 0, zIndex: 50, display: 'flex', alignItems: 'flex-end', justifyContent: 'center', background: 'rgba(43,42,51,.35)' }}
      >
        <div
          role="dialog"
          aria-modal="true"
          aria-label={place?.name ?? t.pdp.back}
  ...
  return (
    // PO 피드백 #6: PDP = 바텀시트 프레젠테이션. 직접 진입/새로고침은 route 유지(딥링크·Phase 2 호환), 시트 자체 높이를 고정해 내부만 스크롤한다.
    <main style={{ maxWidth: 460, margin: '0 auto', height: '100dvh', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
      <Aurora />
      <div
        aria-hidden="true"
        onClick={goBack}
